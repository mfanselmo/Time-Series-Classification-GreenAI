...
onMounted(() => {
    if (props.options.length === 1) emit('update:selected', props.options[0].value)
})

const showCurrentOrPlaceholder = computed(() => {
    if (!props.selected) return props.placeholder

    return props.options.find(o => o.value === props.selected)?.label || props.selected
})
        ...
        <Listbox :model-value="selected || ''" >
            <div class="relative mt-1">
                <ListboxButton
                    ...
                    <span class="block truncate" :class="!selected && 'text-gray-500'">{{ showCurrentOrPlaceholder }}</span>
                ...
                <transition leave-active-class="transition duration-100 ease-in" leave-from-class="opacity-100"
                    ...
                    <ListboxOptions
                        ...
                        <ListboxOption v-slot="{ active, selected }" v-for="option in options" :key="option.value"
